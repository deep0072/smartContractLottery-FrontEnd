...
export default function ManualHeader() {
    const { enableWeb3, account, isWeb3Enabled, Moralis, deactivateWeb3, isWeb3EnableLoading } =
        useMoralis() // useMoralis() is hook used to change the state of variable and re render this change to front end
    ...
    useEffect(() => {
        ...
        if (typeof window !== "undefined") {
            if (window.localStorage.getItem("connected")) {
                /* check last status of wallet is connected in local storage or  not if connected then run enableweb3
